...
(function(){
	...
	function startProcess(category){
		...
		_getData().then(function(data){
			if (data.length){
				console.info("already have a raw file");
		...
		util.logger.log("info", "Processs started for " +  category.name);
	...
	function _init(){
		util.init();
		util.logger.log('info', "Job Details", {searchIndex: util.getSearchHostIndex(), dynamoEndpoint: util.getDynamoEndpoint(), searchIndexHost: util.getSearchHostPath()}) ;
	}
