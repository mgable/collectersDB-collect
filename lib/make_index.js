(function(){
	...
	function makeIndex(_diff, index, host, type, complete){
		...
		if (!util.program.noindex){
			...
			util.logger.log("warn", "In NO-INDEX mode. Not making Index.");
	...
	function _makeSuggestField(item){
		var config = util.getConfigValue("output"),
			category = util.getCurrentCategory(),
			stopWords = _.union((category.stopWords || []), config.stopWords.global),
	...
	function _indexExists(index, items, cb, type){
		...
		client.indices.exists({index: index}, function(error, exists){
			if (exists === true){
				util.logger.log("verbose", "type exists", {index: index});
				_typeExists(index, type, items, cb);
			} else {
				util.logger.log("warn", "index does not exist", {index: index, type: type});
				_createIndex(index, function(){
					util.logger.log("verbose", "checking type exists");
					_typeExists(index, type, items, cb);
				});
			}
		});
	...
	function _typeExists(index, type, items, cb){
		client.indices.existsType({index: index, type: type}, function(error, exists){
			if (exists === true){
				util.logger.log("verbose", "type exists", {index: index, type: type});
				...
				util.logger.log("warn", "type does not exist", {index: index, type: type});
	...
	function _createIndex(index, cb){
		client.indices.create({index: index}, cb);
		util.logger.log("warn", "creating index", {index: index});
	}
