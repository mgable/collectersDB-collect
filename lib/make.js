...
(function(){
	...
	function bulkImport(index, type, items, cb){
		if (items.length){
			...
			util.logger.log("info", "Bulk Import Completed", {index: index, type: type, itemCount: items.length});
			deferred.resolve();
		}
	}
	...
	function cb(error, data){
		if (data){
			util.logger.log("info", "Created Index", data);
			util.logger.log("info", catIndex());
		} else if (error){
