...
(function(){
	...
	// assignments
	var additionalImages = Q.defer(),
		results = [];

	// public methods
	function fetchPage(page){
		console.info("fetchPage: " + page);
		var deferred = Q.defer(),	
			container = "",
			req = http.request(page, function(res) {
	...
	function _getAdditionalImageData(item, additionalImages){
		console.info("making origin images");
		item.images = {};
	...
	function _getAdditionalImages(completedLink, item){
		var deferred = Q.defer();
		console.info("getting additional images for " + item.id);
		fetchPage(util.makeOptions(completedLink)).then(function (originalPage){
			console.info("GOT originalPage");
	...
	function _process(items){
		if (items.length){
			...
			return _getCompletedPage(item).then(function(link){
				console.info("the completed link is " + link);
	...
	function _getCompletedItemLink(data, link){
		...
		try{
			...
			//TODO util.logger.log("ERROR - Can not find completed link for: " + link, 'error');
