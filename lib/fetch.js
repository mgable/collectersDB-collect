...
(function(){
				...
				if (link){
					return _getAdditionalImages(link, item).then(function(item){
						if(item.images.original.length) {
							...
							if (item.images.fail) {
								util.logger.log('error', 'Failed to fetch additional image (no additional images found) - abort', {itemID: item.id, link: link});
							} else {
								item.images.fail = true;
								util.logger.log('warn', 'Failed to fetch additional image (no additional images found) - retry', {itemID: item.id, link: link});
								items.push(item);
							}
						}
						_process(items);
	...
	function _getCompletedItemLink(data, item){
		util.logger.log("verbose", "getting completed link", {itemID: item.id, link: item.link});
		try{
			...
			util.logger.log("error", "Can not find completed link", {itemID: item.id, link: item.link, data: JSON.stringify(data)});
			return false;
		}
	}
