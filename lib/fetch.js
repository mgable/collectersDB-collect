...
(function(){
	...
	function fetchPage(page, data){
		...
		if (_.isArray(page)){
			...
			// a page to fetch
			util.logger.log("info", "Fetching Page", page);
	...
	function fetchImageData(diff){
		...
		if(_hasImageData(_.last(diff))){
			...
			util.logger.log("info", "Fetching Image Data", {itemCount: diff.length});
			var items = _getThumbnailData(diff);

			return _fetchAdditionalImageData(items).then(function(promises){
				return Q.all(promises).then(function(data){
					util.logger.log("info", "Fetched Additional Images - Completed");
					save.saveDiff(data);
					return data;
				});
			});
	...
	function fetchImages(diff){
		...
		if (util.program.noimages){
			...
			util.logger.log("info", "Fetching Images");
			thumbs.fetchImages(diff, imagePath).then(function(data){
				util.logger.log("info", "Fetched Thumbnails");
		...
		if (items.length){
			...
			return _getCompletedPage(item).then(function(link){
				if (link){
					...
					util.logger.log("error", "Failed to fetch additional images", {itemID: item.id} );
	...
	function _getCompletedItemLink(data, link){
		...
		try{
			...
			util.logger.log("error", "Can not find completed link", {link: link, data: data});
			return false;
		}
	}
